...
@router.message_created(Command("reload_schools"))
async def cmd_reload_schools(event: MessageCreated, session: AsyncSession):
    user_id = event.message.sender.user_id
    if not is_admin(user_id):
        await event.message.answer("У вас нет прав для этой команды.")
        return

    await event.message.answer("\u23f3 Загрузка школ...")
    try:
        schools_result = await reload_schools_data(session)
        await event.message.answer(
            f"\u2705 Регионы: {schools_result['regions']}, "
    ...
    await event.message.answer("\u2705 Загрузка школ завершена!")


@router.message_created(Command("reload_lessons"))
async def cmd_reload_lessons(event: MessageCreated, session: AsyncSession):
    user_id = event.message.sender.user_id
    if not is_admin(user_id):
        await event.message.answer("У вас нет прав для этой команды.")
        return
    ...
    await event.message.answer("\u2705 Загрузка контента завершена!")
